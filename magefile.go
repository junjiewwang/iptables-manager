...
func Build(ctx context.Context) error {
	mg.Deps(Clean)
	// Build sequentially to avoid directory conflicts
	if err := BuildBackend(ctx); err != nil {
		return err
	}
	if err := BuildFrontend(ctx); err != nil {
		return err
	}
	return nil
}
...
func BuildFrontend(ctx context.Context) error {
	...
	if err := sh.RunWithV(env, "npm", "install", "--prefix", frontendDir); err != nil {
		return fmt.Errorf("failed to install frontend dependencies: %w", err)
	}

	// Build frontend
	if err := sh.RunWithV(env, "npm", "run", "build", "--prefix", frontendDir); err != nil {
	...
	// Copy frontend dist to root dist directory
	frontendDistDir := filepath.Join(frontendDir, "dist")
	if err := sh.Run("cp", "-r", frontendDistDir+"/.", distDir); err != nil {
		return fmt.Errorf("failed to copy frontend dist directory: %w", err)
	}
...
func BuildBackend(ctx context.Context) error {
	...
	env := map[string]string{}
	if err := sh.RunWithV(env, "go", "mod", "tidy", "-C", backendDir); err != nil {
		return fmt.Errorf("failed to tidy go modules: %w", err)
	}

	// Build backend and place it in dist directory
	buildEnv := map[string]string{
		"CGO_ENABLED": "1",
		"GOOS":        "linux",
	}
	backendBinary := filepath.Join(distDir, "iptables-backend")
	if err := sh.RunWithV(buildEnv, "go", "build", "-C", backendDir, "-a", "-installsuffix", "cgo", "-o", backendBinary, "."); err != nil {
...
func Clean(ctx context.Context) error {
	...
	// Remove dist directory (contains both frontend and backend artifacts)
	if err := sh.Rm("dist"); err != nil && !os.IsNotExist(err) {
		return fmt.Errorf("failed to remove dist directory: %w", err)
	}

	// Remove legacy backend binary (if exists)
	if err := sh.Rm("iptables-backend"); err != nil && !os.IsNotExist(err) {
		return fmt.Errorf("failed to remove legacy backend binary: %w", err)
	}
